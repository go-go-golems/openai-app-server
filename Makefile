...
release:
	...
	GOWORK=off GOPROXY=proxy.golang.org go list -m github.com/go-go-golems/openai-app-server@$(shell svu current)
...
OPENAI_APP_SERVER_BINARY=$(shell which openai-app-server)
install:
