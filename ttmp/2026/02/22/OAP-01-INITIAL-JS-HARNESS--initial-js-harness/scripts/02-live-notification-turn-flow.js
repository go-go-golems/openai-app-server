...
session.onNotification((evt) => {
  ui.emit({ type: "live-notif", method: evt.method, params: evt.params || {} });
});
...
session.request("thread/start", {
  ...
  ui.emit({ type: "thread-start-result", ok: true, thread });
  ...
  ui.emit({ type: "turn-start-result", ok: true, turn });
}).catch((err) => {
  ui.emit({ type: "second-real-test-error", ok: false, error: String(err) });
});
